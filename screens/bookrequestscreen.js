...
export default class Bookrequest extends Component {
  ...
  createuniqueid(){
    return Math.random().toString(100).substring(1)
  }
  addrequest =(bookname,reason)=>{
    var userid = this.state.userid
    var requestidgenerator = this.createuniqueid()
    db.collection('requestbooks').add({"userid":userid,"bookname":bookname,"reason":reason,"uniqueid":requestidgenerator})
    this.setState({bookname:'',reason:''})
    return Alert.alert("you have succefully redeemed the book")
  }
    render(){
  return (
    <View style={{flex:1}}>
      ...
      <KeyboardAvoidingView style ={styles.keyboardstyle}>
        ...
        <TouchableOpacity styles = {styles.button}onPress={()=>{this.addrequest(this.state.bookname,this.state.reason)}}>
        </TouchableOpacity>
